...
public class ContestRepository implements IContestRepository {
    ...
    public Contest save(Contest entity) {
        if( entity.getId() == null ){
            autoIncrement++;
            Contest c = new Contest(autoIncrement+"",entity.getName(),entity.getQuestions(),entity.getLevel(),entity.getCreator(),entity.getContestStatus());
    ...
    public List<Contest> findAll() {
        ...
        {
            allContests.add(entry.getValue());
        }
     return allContests;
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        List<Contest>  allContestsLevelWise = new ArrayList<Contest>();
        if(level == null)
        {
            return findAll();
        }
        ...
        for(Map.Entry<String,Contest> entry : contestMap.entrySet())
        {
            Contest c = entry.getValue();
            if(c.getLevel() == level)
            {
                allContestsLevelWise.add(c);
            }
